let open = document.getElementById('open-btn'),
				...
				insert_discount = document.querySelector('.insert-discount'),
...
//Вводим полученную скидку, выводим её в поле слева и окрашиваем поле в зелёный цвет
insert_discount.addEventListener('change', () => {
	let discount = insert_discount.value;
	if (!isNaN(discount) && discount != '' && discount <= 20 && discount != 0) {
		discount_value.textContent = discount + '%';
		discount_value.style.backgroundColor = 'green';
	} else {
			discount_value.textContent = '';
			insert_discount.value = '';
